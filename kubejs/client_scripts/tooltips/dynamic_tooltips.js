...
const item_modifier_tooltips = [Component.translate('item.modifiers.any').getString(), Component.translate('item.modifiers.armor').getString(),
    ...
    */
    ItemEvents.dynamicTooltips('organize_equipment_tooltip', event => {
        ...
        let relicInfoIdx = -1;
        let researchProgressIdx = -1;
        let ringAffinityIdx = -1;
        let projectileIdx = -1;
        let bannerPatternStartIdx = -1;
        let bannerPatternCount = 0;
        ...
        for (let i in event.lines) {  // Scan the entire tooltip first
          ...
          Siblings: ${event.lines[i].getSiblings()}`);
            ...
            if (line.getString().includes("|||||")) researchProgressIdx = i;
            ...
            if (tKey != undefined) {
                ...
                else if ((spellImbueMarkerIdx > -1 || item.id == "irons_spellbooks:hither_thither_wand") && tKey == "tooltip.irons_spellbooks.imbued_tooltip" && selectedSpellStart == -1) {
                ...
                // Item is a relic
                else if (tKey == "relics.description.researching.info") {
                    relicInfoIdx = i;
                }
                // Affinity ring attunement
                else if (tKey == "tooltip.irons_spellbooks.empty_affinity_ring" || tKey == "tooltip.irons_spellbooks.enhance_spell_level" || tKey == "tooltip.irons_spellbooks.enhance_spell_level_plural") {
                    ringAffinityIdx = i;
                }
                // Crossbow projectile
                else if (tKey == "item.minecraft.crossbow.projectile" || tKey == "item.iss_magicfromtheeast.repeating_crossbow.ammo_amount") {
                    projectileIdx = i;
                }
                // Line is a banner pattern
                else if (tKey.startsWith("block.minecraft.banner")) {
                    if (bannerPatternStartIdx == -1)
                        bannerPatternStartIdx = i;
                    bannerPatternCount += 1;
                }
            }
        }
        ...
        */
        ...
        // Iron's spellbooks affinity ring attunements
        if (ringAffinityIdx > -1) {
            event.lines.set(currIdx, linesCopy[ringAffinityIdx]);
            currIdx += 1;
        }
        ...
        if (!event.alt) {  // Hide these categories when pressing alt
            ...
            // Projectile information for crossbows
            if (projectileIdx > -1) {
                event.lines.set(currIdx, linesCopy[projectileIdx]);
                currIdx += 1;
                event.lines.set(currIdx, Component.empty());
                currIdx += 1;
            }
            // Any special ability information for certain items
            let addInfo = getAdditionalInfo(item.id, event.shift);
            for (let i = 0; i < addInfo.length; i++) {
                event.lines.set(currIdx, addInfo[i]);
                currIdx += 1;
            }
            if (addInfo.length > 0) {
                event.lines.set(currIdx, Component.empty());
            ...
            if (unconventionalAttr.length > 0) {
                event.lines.set(currIdx, Component.empty());
                currIdx += 1; 
            }
            // Relic research
            if (relicInfoIdx > -1 && researchProgressIdx > -1) {
                event.lines.set(currIdx, linesCopy[relicInfoIdx]);
                currIdx += 1;
                event.lines.set(currIdx, linesCopy[researchProgressIdx]);
                currIdx += 1;
                event.lines.set(currIdx, Component.empty());
                currIdx += 1;
            }
            ...
            if (jewelryHeaderIdx > -1) {
                ...
                if (event.shift) {
                    ...
                    let jewelryLineCount = parseInt(jewelryPartCount) * 2;
                    if (jewelryPattern == "pattern.irons_jewelry.haggler_ring.item" || jewelryPattern == "pattern.irons_jewelry.tearstone_ring.item")
                        jewelryLineCount -= 1;
                    else if (jewelryPattern == "piglin_signet")
                        jewelryLineCount -= 2;

                    for (let i = parseInt(jewelryHeaderIdx) + 1; i < event.lines.length && i < (parseInt(jewelryHeaderIdx) + jewelryLineCount + 1); i++) {
            ...
            // Spells
            if (spellImbueMarkerIdx > -1 || item.id == "irons_spellbooks:hither_thither_wand") {
                let spellLvl = -1;
                let spellName = "";
            ...
            let hasTrim = false; let hasRune = false; let hasBanner = false;
            let reservedIdx = currIdx;
            let reservedSpacer = currIdx + 1;
            currIdx += 2;
            // Armor Trim
            ...
            if (bannerPatternStartIdx > -1) {
                hasBanner = true;
                event.lines.set(currIdx, Component.translate("tooltip.insurgence.format.banner_pattern").color("#AAAAAA"));
                currIdx += 1;
                for (let i = bannerPatternStartIdx; i < bannerPatternCount; i++) {
                    let patternName = linesCopy[i].getContents().key;
                    event.lines.set(currIdx, Component.translate(patternName).color("#555555"));
                    currIdx += 1;
                }
                event.lines.set(currIdx, Component.empty());
                currIdx += 1;
            ...
            // Message saying no cosmetics have been applied
            if (!hasTrim && !hasRune && !hasBanner) {
                event.lines.set(reservedIdx, Component.translate("tooltip.insurgence.format.no_cosmetics").color("#FFFF55"));
                event.lines.set(reservedSpacer, Component.empty());
            }
            else {
                event.lines.set(reservedIdx, Component.translate("tooltip.insurgence.format.cosmetics").color("#FFFF55"));
                event.lines.set(reservedSpacer, Component.empty());
            }
        }
        ...
        let addLore = getAdditionalLore(item.id);
        for (let i = 0; i < addLore.length; i++) {
            event.lines.set(currIdx, addLore[i]);
            currIdx += 1;
        }
        if (addLore.length > 0) {
            event.lines.set(currIdx, Component.empty());
